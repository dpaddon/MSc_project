...
def main(_):
    ...
    for d_s in datasets:
        writer = tf.python_io.TFRecordWriter(FLAGS.output_path + '/' + d_s)
                
        
        frames = [f for f in os.listdir(abs_path + '/' + d_s) if not f.startswith('.')]
        frames = sorted(frames, key=int)
        
        for frame_num in frames:
            print(frame_num)
            tf_example = create_tf_example(abs_path, frame_num)
